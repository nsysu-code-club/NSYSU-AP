...
CourseData parseCourseData(
  ...
  final String lang = languageCode ?? Intl.defaultLocale ?? 'zh';
...
/// Result of parsing score HTML — scores list and detail, without pre-score lookup.
class ParsedScoreResult {
  final List<Score> scores;
  final Detail detail;
  /// Course numbers that have no final score (candidates for pre-score lookup).
...
/// Parse score data HTML into [ParsedScoreResult].
///
/// Does NOT perform pre-score lookup — returns [missingFinalScoreCourseNumbers]
/// for the caller to handle async lookup separately.
...
/// Returns null if the HTML does not contain a valid report.
...
/// Returns null if the HTML indicates no data.
